...
**Target audience:** Master 1 students discovering Prolog programming  
**Project type:** Minimal CPU-based Raytracer in Prolog  
...
- Be written **entirely in standard Prolog 3.x**  
...
/project-root
│
├── raytrace.pl    // Entry point; handles image buffer, invokes renderer
├── tracer.pl      // Raytracing core: intersections, shading, lighting
├── scene.pl       // Scene definition: where the user places objects
└── output.tga    // Generated image (result)
```

There is a sample `scene.c` file in this directory. You must read it to reproduce the default scene in Prolog.
...
Shading uses a simplified **"Principled PBR"** model inspired by Blender, but implemented directly in Prolog (no external math libs).  
...
All math should be written in plain Prolog — no external libs.

---

## 8. Scene Definition

All scene setup happens in `scene.pl`:

- Create objects by filling an array of `Primitive` structs
- Each `Primitive` has a position, size, material, and type
- Example default scene: a **cube on a checkeploard plane**, one point light, camera at origin

Example in C (but use Prolog):

```c
void setup_scene(Scene* scene) {
    // Ground
    scene->objects[0] = make_plane(vec3(0,-1,0), vec3(0,1,0), material_checkeploard());
...
- Understand Prolog 
- Practice modular design through `.pl` files  
...
1. Write their own `scene.pl` (personalized scene)
...
|------------|---------|-------------|
| Code structure | 25% | Proper separation in `.pl` files |
